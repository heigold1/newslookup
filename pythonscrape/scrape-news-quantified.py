...
def scrape_news(symbol, yesterday_days):
    ...
    for row in news_section:
        ...
        if re.search(r'class.action', headline, re.IGNORECASE) or re.search(r"ATTENTION .+? SHAREHOLDERS", headline) or re.search(r"ALERT:", headline) or re.search(r"investors who have lost money", headline, re.IGNORECASE) or re.search(r"shareholders who lost money", headline, re.IGNORECASE) or re.search(r"contact .+? regarding an ongoing investigation", headline, re.IGNORECASE) or re.search(r"investigated by the .+? law firm", headline, re.IGNORECASE)  :
            continue 
