		...
		switch($choice){
			case "1":
				...
				if(isset($access_token['oauth_token']) and isset($access_token['oauth_token_secret']))
				{
					...
					$config = new Config_Lite('../etrade.ini', LOCK_EX);
					$config->set('OAuth', 'oauth_token', $access_token['oauth_token']);
					$config->set('OAuth', 'oauth_token_secret', $access_token['oauth_token_secret']);
					$config->save();
